...
def makeBinary(inFile):
    ...
    os.system('python2 declump_bridge.py "{0}" "{1}" "{2}"'.format(inFile, inFile.replace('.tif', '_Binary.tif'),
                                                                   inFile.replace('.tif', '_labeled.pkl')))
...
def visualize_labeled(labeled, inFile, name='_labeled'):
    ...
    colored = ([0, 0, colorLimit], [0, colorLimit, 0], [colorLimit, 0, 0], [colorLimit, colorLimit, 0],
               [colorLimit, 0, colorLimit], [0, colorLimit, colorLimit], [colorLimit, colorLimit, colorLimit])
...
def parallel(prefix, binarized, clustered, split, overlaid):
    current_stack = Stack(512, 512, 64)
...
def declumpKMeansBoundary(inFile, clusters):
    ...
    colored = ([0, colorLimit, 0], [colorLimit, 0, 0], [colorLimit, colorLimit, 0], [colorLimit, 0, colorLimit],
               [0, colorLimit, colorLimit], [colorLimit, colorLimit, colorLimit])
...
def visualize_Cells(out_array, prefix, cells):
    c_arr = copy.deepcopy(out_array)  # psuedo-deep
    x = 1
    for k in range(len(cells)):
        for i in range(len(c_arr)):
            for j in range(len(c_arr[0])):
                c_arr[i][j] = 0
        ck = [(c[0], c[1]) for c in cells[k].boundary]
        for (i, j) in ck:
            c_arr[i][j] = WHITE
        skimage.external.tifffile.imsave(prefix + '_cells_' + str(x) + '.tif', c_arr)
        x += 1
...
def overlay(current_stack, prefix, pic_arrays):
    ...
    colored = ([0, 0, colorLimit], [0, colorLimit, 0], [colorLimit, 0, 0], [colorLimit, colorLimit, 0],
               [colorLimit, 0, colorLimit], [0, colorLimit, colorLimit], [colorLimit, colorLimit, colorLimit])
    cyan = [0, colorLimit, colorLimit];
    magenta = [colorLimit, 0, colorLimit];
    yellow = [colorLimit, colorLimit, 0]
    green = [0, colorLimit, 0];
    white_ = [colorLimit, colorLimit, colorLimit];
    red = [colorLimit, 0, 0]
    blue = [0, 0, colorLimit]
    ...
    x = 1
    k = 1
    assert len(current_stack.stack_slices) == len(
        pic_arrays), 'Stack slices and picture array mismatches: {0} and {1}'.format(len(current_stack.stack_slices),
                                                                                     len(pic_arrays))
    for ss, pic_array in zip(current_stack.stack_slices, pic_arrays):
        assert ss.number == k, 'Stack slice pic array mismatch'
        k = k + 1
        ...
        for c in ss.overlapping_Cells:
            for b in c.boundary:
                largest_3d[-1][b[0]][b[1]] = [int(pic_array[b[0]][b[1]] * 0.5) + [int(c * 0.5) for c in white_][i] for i
                                              in range(0, 3)]
        for c in ss.roundness_rejected_Cells:  # this is a subset of cells (above) so order matters!
            for b in c.boundary:
                largest_3d[-1][b[0]][b[1]] = [int(pic_array[b[0]][b[1]] * 0.5) + [int(c * 0.5) for c in yellow][i] for i
                                              in range(0, 3)]
        for c in ss.size_rejected_Cells:
            for b in c.boundary:
                largest_3d[-1][b[0]][b[1]] = [int(pic_array[b[0]][b[1]] * 0.5) + [int(c * 0.5) for c in blue][i] for i
                                              in range(0, 3)]
        ...
        for c in ss.finalizedCells:
            outFile.write("Cell, {0}, Slice #, {1}, Area:, {2}, Roundness:, {3}, Viral status:, {4}\n".format(x,
                                                                                                              ...
                                                                                                              c.area * (
                                                                                                                          212 / 512) ** 2,
                                                                                                              c.roundness,
                                                                                                              virus_inject and c.isLoaded))
            x += 1
            for b in c.interior:
                largest_3d[-1][b[0]][b[1]] = [int(pic_array[b[0]][b[1]] * 0.8) + [int(c * 0.2) for c in cyan][i] for i
                                              in range(0, 3)]
            if virus_inject and c.isLoaded:
                for b in c.boundary:
                    largest_3d[-1][b[0]][b[1]] = [int(pic_array[b[0]][b[1]] * 0.5) + [int(c * 0.5) for c in red][i] for
                                                  i in range(0, 3)]
            else:
                for b in c.boundary:
                    largest_3d[-1][b[0]][b[1]] = [int(pic_array[b[0]][b[1]] * 0.5) + [int(c * 0.5) for c in cyan][i] for
                                                  i in range(0, 3)]
