...
class Cluster:
    ...
    def __init__(self, binary, boundary, stack_slice, pivots=None, internalEdges=None):
        ...
        self.internalEdges = [] if (
                    internalEdges is None) else internalEdges  # using [] as default argument is problematic; old is appended to default
    ...
    def getTrueCusps(self, segmentLen=8, arc=None):
        ...
        for point in arc:
            ...
            for segmentPoint in range((segmentLen // 4) * 3, 1 + segmentLen):
                ...
                midpt = (math.floor(np.mean([p[0] for p in [before, after]])),
                         math.floor(np.mean([p[1] for p in [before, after]])))
    ...
    def pruneCusps(self):
        ...
        while self.cusps:
            ...
            while k < len(self.cusps) and max(abs(self.cusps[k].point[0] - previous.point[0]),
                                              abs(self.cusps[k].point[1] - previous.point[1])) <= 1:
    ...
    def growPivots(self):
        ...
        for p in self.pivots:
            ...
            i = 0
            while i < l:
                if ((i + 1) < l and minima[(i + 1)] - minima[i] < 40) or (
                        (i + 1) > l and minima[(i + 1) % l] + l - minima[i] < 40):  # modulus allows wrapping
                    group = [i % l, (i + 1) % l]
                    k = 2
                    while ((i + k) < l and minima[(i + k)] - minima[i] < 40) or (
                            (i + k) > l and minima[(i + k) % l] + l - minima[i] < 40):
                        group.append((i + k) % l)
                        k += 1
    ...
    def propagateInternalBoundaries(self, cleave_points=None):
        ...
        for cp in cleave_points:
            ...
            orientation = lambda p: abs(np.mean([(math.atan(p.left_deriv) - math.atan(cp.left_deriv)),
                                                 (math.atan(p.right_deriv) - math.atan(cp.right_deriv))]))
            viable = filter(lambda p: abs(orientation(p)) > math.pi * 0.5, cleave_points)
            viable_no_duplicate = filter(lambda p: (p, cp) not in completed_pairs and (cp, p) not in completed_pairs,
                                         viable)
            try:
                pair = min(viable_no_duplicate,
                           key=lambda p: (cp.point[0] - p.point[0]) ** 2 + (cp.point[1] - p.point[1]) ** 2)
            except ValueError:
                continue
        ...
        if self.internalEdges == []:
            ...
            if self.internalEdges:
                ...
                if start_index > end_index:
                    cell_1_bound = list(reversed(divider.internalEdge)) + self.boundary[start_index: end_index: -1] + [
                        divider.end]
                    cell_2_bound = list(reversed(divider.internalEdge)) + self.boundary[start_index:] + self.boundary[
                                                                                                        :end_index + 1]

                else:
                    cell_1_bound = list(reversed(divider.internalEdge)) + self.boundary[start_index: end_index: 1] + [
                        divider.end]
                    cell_2_bound = list(reversed(divider.internalEdge)) + self.boundary[end_index::1] + self.boundary[
                                                                                                        :start_index + 1:1]
    ...
    def getBoundary2D(self):
        ...
        return [list(filter(lambda p: p[0] == y, sortedBound)) for y in
                range(sortedBound[0][0], sortedBound[-1][0] + 1)]
    ...
    def get_var_pairs(self, var_bounds, index, area=False):  # if area is true check for controversial region
        ...
        while k < len(var_bounds) - 1:
            ...
            while btwn < var_bounds[k + 1][index]:
