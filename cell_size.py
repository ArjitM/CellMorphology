...
def watershedBoundaries(thresh, img):
    ...
    unknown = cv2.subtract(sure_bg, sure_fg) #img_as_ubyte(skimage.filters.sobel(img)) #
    ...
    for i in range(int(np.max(relabeled)) + 1):
        ...
        if area < MIN_AREA or area > MAX_AREA or score < ROUNDNESS_THRESH:
            relabeled[closing > 0] = 0
        else:
            relabeled[closing > 0] = i
            areas.append(area)

    relabeled = img_as_ubyte(relabeled > 0)
    relabeled, _ = ndi.measurements.label(relabeled)

    return relabeled, np.array(areas)
...
def processStack(prefix):
    ...
    while True:
        try:
            ...
            # print("Slice {0} area: {1}".format(x, np.mean(areas)))
    ...
    flab = img_as_ubyte(finalZSlice.labels > 0)
    flab, _ = ndi.measurements.label(flab)
    fVis = skimage.color.label2rgb(flab, bg_label=0)
...
def collate(sliceStack):
    """
    Given a 3D stack of images whose objects are labeled, save only objects on slices where object area is maximal.
    If an object is segmented into several objects on another slice, retain the large unsegmented object to prevent
    oversegmentation.
...
if args.localRun:
    ...
    for p in getImageDirectories(["/Users/arjitmisra/Documents/Kramer_Lab/RAW/RAW/"]):
        try:
            processStack(p)
        except Exception as e:
            print("Error occured in processing {0}: {1}".format(p, e))
            logging.error(traceback.format_exc())
    #processStack("/Users/arjitmisra/Documents/Kramer_Lab/RAW/RAW/RD1/expt_2/piece1-gfp-normal3/")
